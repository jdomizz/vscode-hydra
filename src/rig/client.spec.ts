...
describe("RigWire", () => {
  ...
  it("throws when sendCommand is called before connect()", async () => {
    const wire = new RigWire();
    await expect(wire.sendCommand({ type: "eval:code", code: "osc().out()" })).rejects.toThrow(
      /not connected/i,
    );
    wire.dispose();
  });
  ...
  it("eval() correlates response by id and returns ok:true on state feedback", async () => {
    ...
    h.sockets[0].receive(JSON.stringify({ id: sent.id, type: "state", playing: true, bpm: 120 }));
  ...
  it("sendCommand() passes through to TransportClient", async () => {
    ...
    h.sockets[0].receive(JSON.stringify({ id: sent.id, type: "panic:state", active: true }));
  ...
  it("onFeedback() receives push events (feedback without id)", () => {
    ...
    h.sockets[0].receive(JSON.stringify({ type: "fps", value: 60 }));
