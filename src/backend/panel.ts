...
export class HydraPanel {
  ...
  private onMessage(message: { type: string; value: string }) {
    switch (message.type) {
      case "status":
        return vscode.commands.executeCommand("setContext", "vscode-hydra.status", message.value);
      case "error":
        return vscode.window.showErrorMessage(message.value);
      case "start":
        return this.panel?.webview.postMessage({ type: "evalCode", value: this.code });
    }
  }
