...
function makeFakeElement() {
  ...
  return element as typeof element & { code: string; _setEvalResult: (r: { success: boolean; error?: string }) => void };
}
...
describe('createHydraEngine (adapter)', () => {
  ...
  it('captureImage() calls screencap and returns canvas blob', async () => {
    const el = makeFakeElement();
    const engine = createHydraEngine(el as unknown as HTMLElement);
    const blob = await engine.captureImage!();
...
describe('runtime-conformance (rig-host + adapter-shaped engine)', () => {
  ...
  it('capture:image routes to engine.captureImage', async () => {
    const captureSpy = vi.fn().mockResolvedValue(new Blob());
